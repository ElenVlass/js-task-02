...
const getNamesSortedByFriendsCount = users => [...users].sort((user1, user2) => user1.friends.length - user2.friends.length)
console.table(getNamesSortedByFriendsCount(users)); // [ 'Moore Hensley', 'Sharlene Bush', 'Elma Head', 'Carey Barr', 'Blackburn Dotson', 'Sheree Anthony', 'Ross Vazquez' ]

const getSortedUniqueSkills = users => users.flatMap(user => user.skills).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();
